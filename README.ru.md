...
По умолчаню мой логер отображает дату в следующем формате:
...
Для того, чтобы отображать сообщения как раньше, нужно ввести:
...
Если же мы снова хотим её включить, то нужно ввести:

```js
logger.enableDate();
```

Логично, что если возможно отключать, или включать дату, то нужно иметь возможность включать, или отключать время. Для отключения времени нужно ввести:

```js
logger.disableTime();
```

Для включения времени, нужно ввести:

```js
logger.enableTime();
```

Если нам полностью нужно отключить и время и дату, то нужно ввести:

```js
logger.disableTimePeriod();
```

Для включения времени и даты, то нужно ввести:
...
Если нам нужно отключить подсветку сообщений, то нужно ввести:
